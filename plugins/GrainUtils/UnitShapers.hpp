...
namespace UnitShapers {
    ...
    inline float tukey(float phase, float width) {
        float sustain = 1.0f - width;
        float safeSustain = std::max(sustain, SAFE_DENOM_EPSILON);
        float trapezoidPhase = sc_clip(phase / safeSustain, 0.0f, 1.0f);
        float hanningValue = hanning(trapezoidPhase);
        
        // If phase < sustain, use hanning, otherwise output 1
        return (phase < sustain) ? hanningValue : 1.0f;
    }
