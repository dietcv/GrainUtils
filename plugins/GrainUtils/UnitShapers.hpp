...
namespace WindowFunctions {
    ...
    inline float planckWindow(float phase, float skew, float width, float index) {
        float warpedPhase = UnitShapers::triangle(phase, skew);
        return UnitShapers::planck(warpedPhase, width, index);
    }
...
class PlanckWindow : public SCUnit {
public:
    PlanckWindow();
private:
    void next(int nSamples);
