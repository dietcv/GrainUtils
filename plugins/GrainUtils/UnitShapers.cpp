...
PlanckWindow::PlanckWindow() {
    mCalcFunc = make_calc_function<PlanckWindow, &PlanckWindow::next>();
    next(1);
}

void PlanckWindow::next(int nSamples) {
    const float* phaseIn = in(0);
    const float* skewIn = in(1);
    const float* widthIn = in(2);
    const float* indexIn = in(3);
    float* outbuf = out(0);
    ...
    for (int i = 0; i < nSamples; ++i) {
        ...
        float width = sc_clip(widthIn[i], 0.0f, 1.0f);
        float index = sc_clip(indexIn[i], 0.0f, 1.0f);
        outbuf[i] = WindowFunctions::planckWindow(phase, skew, width, index);
    }
}
