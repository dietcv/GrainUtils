...
struct VoiceAllocator {
    ...
    std::vector<double> localPhases;      // Internal double-precision phases
    std::vector<double> localSlopes;      // Rate/sampleRate for each channel
    std::vector<bool> isActive;           // Which channels are currently active
    std::vector<bool> justTriggered;      // Internal trigger tracking
    
    // Output interface
    std::vector<float> phases;            // Phases for external use
    std::vector<bool> triggers;           // Triggers for external use
    int numChannels;
    
    explicit VoiceAllocator(int channels = 16) : numChannels(channels) {
        localPhases.resize(numChannels, 0.0f);
        localSlopes.resize(numChannels, 0.0f);
    ...
    void process(bool trigger, float rate, float subSampleOffset, float sampleRate) {
        ...
        for (int ch = 0; ch < numChannels; ++ch) {
            ...
            if (localPhases[ch] >= 1.0f) {
                isActive[ch] = false;
                phases[ch] = 0.0f;
                triggers[ch] = false;
            } else {
                phases[ch] = static_cast<float>(localPhases[ch]);
                triggers[ch] = justTriggered[ch];
            }
        }
    }
    
    void reset() {
        std::fill(localPhases.begin(), localPhases.end(), 0.0f);
        std::fill(localSlopes.begin(), localSlopes.end(), 0.0f);
