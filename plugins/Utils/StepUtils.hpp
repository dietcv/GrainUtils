...
namespace UnitSteps {
    ...
    struct UnitUrn {
        ...
        float process(float phase, float chance, int length, bool resetTrigger, RGen& rgen) {
            ...
            // Rebuild deck if length changed
            if (length != m_length) {
                m_initialized = false;
            }
            ...
            if (trigger) {
                ...
                if (rgen.frand() < chance) {
                    int remaining = m_length - m_position;
                ...
                m_output = static_cast<float>(m_lastDrawn) / (m_length - 1);
            }
